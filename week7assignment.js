...
for (let i = 0; i < names.length; i++) {
  //this will count upwards as long as i is less than the length of the array,
...
for (let i = 0; i < names.length; i++) {
  // this goes through the length of the array again, from zero to the maximum.
...
for (let i = 0; i < nameLengths.length; i++) {
  //while i is less than the length of the array, add to i.
...
let repeatWord = (word, n) => {
  ...
  for (let i = 0; i < n; i++) {
    // if the variable is less than the given number (n), it will keep repeating until it is equal.
    result += word;
  }
...
let oneHundredPlease = (arrayName) => {
  ...
  if (total >= 100) {
    //if/else loop for returning total of whatever array is entered for the parameter of oneHundredPlease.
    return true;
  } else {
    return false;
  }
